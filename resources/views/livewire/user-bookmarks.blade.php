<section class="panelV2 user-bookmarks">
    ...
    <div class="data-table-wrapper">
        <table class="data-table">
            <thead>
                <th class="user-bookmarks__name-header" wire:click="sortBy('torrents.name')"
                ...
                <th class="user-bookmarks__size-header" wire:click="sortBy('torrents.size')"
                    role="columnheader button">
                    {{ __('torrent.size') }}
                    @include('livewire.includes._sort-icon', [
                        'field' => 'torrents.size',
                ...
                <th class="user-bookmarks__seeders-header" wire:click="sortBy('torrents.seeders')"
                    role="columnheader button" title="{{ __('torrent.seeders') }}">
                    <i class="fas fa-arrow-alt-circle-up"></i>
                    @include('livewire.includes._sort-icon', [
                        'field' => 'torrents.seeders',
                    ])
                </th>
                <th class="user-bookmarks__leechers-header" wire:click="sortBy('torrents.leechers')"
                    role="columnheader button" title="{{ __('torrent.leechers') }}">
                    <i class="fas fa-arrow-alt-circle-down"></i>
                    @include('livewire.includes._sort-icon', [
                        'field' => 'torrents.leechers',
                    ])
                </th>
                <th class="user-bookmarks__times-completed-header"
                    wire:click="sortBy('torrents.times_completed')" role="columnheader button"
                    title="{{ __('torrent.completed') }}">
                    <i class="fas fa-check-circle"></i>
                    @include('livewire.includes._sort-icon', [
                        'field' => 'torrents.times_completed',
                    ])
                </th>
                <th class="user-bookmarks__created-at-header"
                    wire:click="sortBy('torrents.created_at')" role="columnheader button">
                    Uploaded at
                    @include('livewire.includes._sort-icon', [
                        'field' => 'torrents.created_at',
                    ])
                </th>
                <th class="user-bookmarks__created-at-header"
                    wire:click="sortBy('bookmarks.created_at')" role="columnheader button">
                    Bookmarked at
                    @include('livewire.includes._sort-icon', [
                        'field' => 'bookmarks.created_at',
                    ])
                </th>
                ...
                @foreach ($bookmarks as $bookmark)
                    <tr>
                        ...
                        <td @class([
                            ...
                            @if ($bookmark->seeding) title="{{ __('torrent.currently-seeding') }}" @endif>
                        ...
                        <td @class([
                            ...
                            @if ($bookmark->leeching) title="{{ __('torrent.currently-leeching') }}" @endif>
                        ...
                        <td @class([
                            ...
                            @if ($bookmark->completed) title="{{ __('torrent.completed') }}" @endif>
                        ...
                        <td class="user-bookmarks__created-at">
                            <time datetime="{{ $bookmark->torrent_created_at }}"
                                title="{{ $bookmark->torrent_created_at }}">
                                {{ $bookmark->torrent_created_at->diffForHumans() }}
                            </time>
                        </td>
                        <td class="user-bookmarks__created-at">
                            <time datetime="{{ $bookmark->bookmark_created_at }}"
                            ...
                            <menu class="data-table__actions">
                                <li class="data-table__action">
                                    <button class="form__standard-icon-button"
                                        x-data="bookmark({{ $bookmark->torrent_id }}, true)" x-bind="button">
                                        <i class="{{ config('other.font-awesome') }}"
                                            x-bind="icon"></i>
                                    </button>
                                </li>
                                <li class="data-table__action">
