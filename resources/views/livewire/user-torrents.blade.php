                        ...
                        <div class="form__fieldset-checkbox-container">
                            <p class="form__group">
                                <label style="user-select: none" class="form__label"
                                    ...
                                    <input type="checkbox" class="user-torrents__checkbox"
                                        x-bind="input" />
                        ...
                        <div class="form__fieldset-checkbox-container">
                            <p class="form__group">
                                <label class="form__label">
                                    <input class="user-torrents__checkbox" type="checkbox"
    ...
    <section class="panelV2 user-torrents">
        ...
        <div class="data-table-wrapper">
            <table class="data-table">
                <thead>
                    <th class="user-torrents__name-header" wire:click="sortBy('name')"
                        role="columnheader button">
                        {{ __('torrent.name') }}
                        @include('livewire.includes._sort-icon', ['field' => 'name'])
                    </th>
                    <th class="user-torrents__seeders-header" wire:click="sortBy('seeders')"
                    ...
                    <th class="user-torrents__leechers-header" wire:click="sortBy('leechers')"
                    ...
                    <th class="user-torrents__agent-header" wire:click="sortBy('agent')"
                        role="columnheader button">
                        {{ __('torrent.client') }}
                        @include('livewire.includes._sort-icon', ['field' => 'agent'])
                    </th>
                    <th class="user-torrents__size-header" wire:click="sortBy('size')"
                        role="columnheader button">
                        {{ __('torrent.size') }}
                        @include('livewire.includes._sort-icon', ['field' => 'size'])
                    </th>
                    <th class="user-torrents__upload-header"
                        wire:click="sortBy('actual_uploaded')" role="columnheader button">
                        {{ __('common.upload') }}
                        @include('livewire.includes._sort-icon', [
                            'field' => 'actual_uploaded',
                        ])
                    </th>
                    <th class="user-torrents__download-header"
                        wire:click="sortBy('actual_downloaded')" role="columnheader button">
                        {{ __('common.download') }}
                        @include('livewire.includes._sort-icon', [
                            'field' => 'actual_downloaded',
                        ])
                    </th>
                    <th class="user-torrents__ratio-header" wire:click="sortBy('actual_ratio')"
                        role="columnheader button">
                        {{ __('common.ratio') }}
                        @include('livewire.includes._sort-icon', [
                            'field' => 'actual_ratio',
                    ...
                    <th class="user-torrents__leechtime-header" wire:click="sortBy('leechtime')"
                        role="columnheader button">
                        Leeched
                        @include('livewire.includes._sort-icon', [
                            'field' => 'leechtime',
                    ...
                    <th class="user-torrents__seedtime-header" wire:click="sortBy('seedtime')"
                        role="columnheader button">
                        Seeded
                        @include('livewire.includes._sort-icon', [
                            'field' => 'seedtime',
                    ...
                    <th class="user-torrents__completed-at-header"
                        wire:click="sortBy('completed_at')" role="columnheader button">
                        {{ __('torrent.completed') }}
                        @include('livewire.includes._sort-icon', [
                            'field' => 'completed_at',
                        ])
                    </th>
                    <th class="user-torrents__prewarned-header"
                        wire:click="sortBy('prewarned_at')" role="columnheader button">
                        {{ __('torrent.prewarn') }}
                        @include('livewire.includes._sort-icon', [
                            'field' => 'prewarned_at',
                        ])
                    </th>
                    <th class="user-torrents__warned-header" wire:click="sortBy('hitrun')"
                        role="columnheader button" title="{{ __('torrent.hitrun') }}">
                        <i class="fas fa-exclamation-triangle"></i>
                    </th>
                    <th class="user-torrents__immune-header" wire:click="sortBy('immune')"
                        role="columnheader button" title="{{ __('torrent.immune') }}">
                        <i class="fas fa-syringe"></i>
                    </th>
                    <th class="user-torrents__upload-header" wire:click="sortBy('uploaded')"
                        role="columnheader button" title="{{ __('torrent.uploaded') }}">
                        <i class="fas fa-upload"></i>
                    </th>
                    <th class="user-torrents__status-header" wire:click="sortBy('status')"
                    ...
                    @foreach ($histories as $history)
                        <tr x-data="userHistory" data-history-id="{{ $history->torrent_id }}">
                            ...
                            <td @class([
                                ...
                                @if ($history->seeding) title="{{ __('torrent.currently-seeding') }}" @endif>
                            ...
                            <td @class([
                                ...
                                @if ($history->leeching) title="{{ __('torrent.currently-leeching') }}" @endif>
                            ...
                            <td @class([
                                ...
                                @if ($history->completed) title="{{ __('torrent.completed') }}" @endif>
                            ...
                            <td class="user-torrents__upload"
                            ...
                            <td class="user-torrents__immune">
                                @if ($history->immune)
                                    <i class="{{ config('other.font-awesome') }} fa-check text-green"
                                        title="Immune"
                                        @if (auth()->user()->group->is_modo) x-on:click.prevent="updateImmune(false)"
                                            data-b64-deletion-message="{{ base64_encode('Are you sure you want to set this history record to not immune: ' . $history->name . '?') }}"
                                            style="cursor: pointer" @endif></i>
                                @else
                                    <i class="{{ config('other.font-awesome') }} fa-times text-red"
                                        title="Not immune"
                                        @if (auth()->user()->group->is_modo) x-on:click.prevent="updateImmune(true)"
    ...
    <script nonce="{{ HDVinnie\SecureHeaders\SecureHeaders::nonce('script') }}">
        document.addEventListener('alpine:init', () => {
            Alpine.data('userHistory', () => ({
                updateImmune(immune) {
                    this.confirmAction(() =>
                        this.$wire.updateImmune(this.$root.dataset.historyId,
                            immune),
                    );
                },
