<section class="panelV2" x-data="tabs($wire.entangle('occupationId').live)">
    ...
    <menu class="panel__tabs">
        <li x-bind="tabButton" x-cloak data-tab="{{ App\Enums\Occupation::CREATOR->value }}"
            x-show="{{ $createdCount }} > 0">
            Creator ({{ $createdCount }})
        </li>
        <li x-bind="tabButton" x-cloak data-tab="{{ App\Enums\Occupation::DIRECTOR->value }}"
            x-show="{{ $directedCount }} > 0">
            Director ({{ $directedCount }})
        </li>
        <li x-bind="tabButton" x-cloak data-tab="{{ App\Enums\Occupation::WRITER->value }}"
            x-show="{{ $writtenCount }} > 0">
            Writer ({{ $writtenCount }})
        </li>
        <li x-bind="tabButton" x-cloak data-tab="{{ App\Enums\Occupation::PRODUCER->value }}"
            x-show="{{ $producedCount }} > 0">
            Producer ({{ $producedCount }})
        </li>
        <li x-bind="tabButton" x-cloak data-tab="{{ App\Enums\Occupation::COMPOSER->value }}"
            x-show="{{ $composedCount }} > 0">
            Composer ({{ $composedCount }})
        </li>
        <li x-bind="tabButton" x-cloak
            data-tab="{{ App\Enums\Occupation::CINEMATOGRAPHER->value }}"
            x-show="{{ $cinematographedCount }} > 0">
            Cinematographer ({{ $cinematographedCount }})
        </li>
        <li x-bind="tabButton" x-cloak data-tab="{{ App\Enums\Occupation::EDITOR->value }}"
        ...
        <li x-bind="tabButton" x-cloak data-tab="{{ App\Enums\Occupation::ART_DIRECTOR->value }}"
            x-show="{{ $artDirectedCount }} > 0">
            Art director ({{ $artDirectedCount }})
        </li>
        <li x-bind="tabButton" x-cloak data-tab="{{ App\Enums\Occupation::ACTOR->value }}"
            x-show="{{ $actedCount }} > 0">
            Actor ({{ $actedCount }})
        </li>
    </menu>
