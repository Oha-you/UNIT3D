            ...
            <a href="{{ route('torrents.show', ['id' => $torrent->id]) }}">
                @switch($media->meta)
                    ...
                    @case('tv')
                        ...
                        @php
                            if (
                                $media->first_air_date?->year !== null &&
                                $media->last_air_date?->year !== null
                            ) {
                                $firstAirDateRange = range(
                                    $media->first_air_date->year - 1,
                                    $media->first_air_date->year + 1,
                                );
                                $fullRange = range(
                                    $media->first_air_date->year - 1,
                                    $media->last_air_date->year + 1,
                                );
                            } else {
                                $firstAirDateRange = [];
                                $fullRange = [];
...
<td class="torrent-search--grouped__bookmark">
    <button x-data="bookmark({{ $torrent->id }}, {{ Js::from($torrent->bookmarks_exists) }})" x-bind="button">
...
<td class="torrent-search--grouped__download">
    ...
    @if (config('torrent.magnet') == 1)
        <a href="magnet:?dn={{ $torrent->name }}&xt=urn:btih:{{ bin2hex($torrent->info_hash) }}&as={{ route('torrent.download.rsskey', ['id' => $torrent->id, 'rsskey' => auth()->user()->rsskey]) }}&tr={{ route('announce', ['passkey' => auth()->user()->passkey]) }}&xl={{ $torrent->size }}"
