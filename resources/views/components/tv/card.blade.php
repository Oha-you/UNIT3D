        ...
        <address class="torrent-search--grouped__directors">
            @if ($media->creators->isNotEmpty())
                ...
                @foreach ($media->creators as $creator)
                    <a href="{{ route('mediahub.persons.show', ['id' => $creator->id, 'occupationId' => App\Enums\Occupation::CREATOR->value]) }}"
    ...
    <section>
        ...
        @if (array_key_exists('Specials', $media->torrents))
            <details class="torrent-search--grouped__dropdown"
                @if (
                    !array_key_exists('Complete Pack', $media->torrents) &&
                        !array_key_exists('Seasons', $media->torrents)) open @endif>
                            ...
                            @foreach ($special as $type => $torrents)
                                <tbody>
                                    @foreach ($torrents as $torrent)
                                        <tr>
                                            @if ($loop->first)
                                                <th class="torrent-search--grouped__type"
                                                    scope="rowgroup" rowspan="{{ $loop->count }}">
                                                    {{ $type }}
                                                </th>
                                            @endif
        ...
        @foreach ($media->torrents['Seasons'] ?? [] as $seasonName => $season)
            <details class="torrent-search--grouped__dropdown"
                ...
                @foreach ($season['Episodes'] ?? [] as $episodeName => $episode)
                    <details class="torrent-search--grouped__dropdown"
                        @if ($loop->first && !array_key_exists('Season Pack', $season)) open @endif>
