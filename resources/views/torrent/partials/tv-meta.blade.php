<section class="meta">
            ...
            <li>
                <a
                    href="{{ route('torrents.create', [
                        ...
                        'title' => rawurlencode(
                            ($meta?->name ?? '') . ' ' . substr($meta->first_air_date ?? '', 0, 4) ?? '',
                        ),
            ...
            @if ($meta?->id || $torrent?->tmdb_tv_id ?? null)
                <li>
                    <form
                        ...
                        @method('PATCH')
                        <button
                            @if (cache()->has('tmdb-tv-scraper:' . ($meta?->id ?? $torrent->tmdb_tv_id))) disabled
    ...
    <ul class="meta__ids">
        @foreach (array_unique(array_filter([$meta->id ?? 0, $torrent->tmdb_tv_id ?? 0])) as $tmdbId)
            <li class="meta__tmdb">
                <a class="meta-id-tag" href="https://www.themoviedb.org/tv/{{ $tmdbId }}"
        ...
        @foreach (array_unique(array_filter([(int) ($meta->tvdb_id ?? 0), $torrent->tvdb ?? 0])) as $tvdbId)
            <li class="meta__tvdb">
                <a class="meta-id-tag"
                    ...
                    title="The TV Database: {{ $tvdbId }}" target="_blank">
            ...
            @foreach ($meta?->networks ?? [] as $network)
                <article class="meta__company">
                    <a class="meta-chip"
                        ...
                        @else
                            <i
                                class="{{ config('other.font-awesome') }} fa-signal-stream meta-chip__icon"></i>
                        @endif
