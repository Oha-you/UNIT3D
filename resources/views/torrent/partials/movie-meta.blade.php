<section class="meta">
    @if ($meta?->backdrop)
        <img class="meta__backdrop" src="{{ tmdb_image('back_big', $meta->backdrop) }}"
            alt="" />
    @endif
    ...
    <a class="meta__poster-link"
        href="{{ $tmdb ? route('torrents.similar', ['category_id' => $category->id, 'tmdb' => $tmdb]) : '#' }}">
        <img src="{{ $meta?->poster ? tmdb_image('poster_big', $meta->poster) : 'https://via.placeholder.com/400x600' }}"
            class="meta__poster" />
    </a>
    <div class="meta__actions">
        ...
        <ul class="meta__dropdown">
            <li>
                <a
                    href="{{ route('torrents.create', [
                        ...
                        'title' => rawurlencode(
                            ($meta?->title ?? '') . ' ' . substr($meta->release_date ?? '', 0, 4) ?? '',
                        ),
            ...
            @if ($meta?->id)
                <li>
                    <form action="{{ route('users.wishes.store', ['user' => auth()->user()]) }}"
                        method="post">
                        @csrf
            ...
            @if ($meta?->id || $torrent?->tmdb_movie_id ?? null)
                <li>
                    <form
                        ...
                        <button
                            @if (cache()->has('tmdb-movie-scraper:' . ($meta?->id ?? $torrent->tmdb_movie_id))) disabled
    ...
    <ul class="meta__ids">
        @foreach (array_unique(array_filter([$meta->id ?? 0, $torrent->tmdb_movie_id ?? 0])) as $tmdbId)
            <li class="meta__tmdb">
                <a class="meta-id-tag" href="https://www.themoviedb.org/movie/{{ $tmdbId }}"
                    title="The Movie Database: {{ $tmdbId }}" target="_blank">
        ...
        @if (($torrent->mal ?? 0) > 0)
            <li class="meta__mal">
                <a class="meta-id-tag" href="https://myanimelist.net/anime/{{ $torrent->mal }}"
                    title="My Anime List: {{ $torrent->mal }}" target="_blank">
        ...
        @if (($torrent->tvdb ?? 0) > 0)
            <li class="meta__tvdb">
                <a class="meta-id-tag"
                    ...
                    title="The TV Database: {{ $torrent->tvdb }}" target="_blank">
        ...
        @if (($meta->imdb_id ?? 0) > 0)
            <li class="meta__blu-ray">
                <a class="meta-id-tag"
                    ...
                    target="_blank">
                    <img class="" src="{{ url('/img/meta/blu-ray.svg') }}"
                        style="width: 40px" />
                </a>
            </li>
        @endif
        ...
        @foreach (array_unique(array_filter([(int) ($meta->imdb_id ?? 0), $torrent->imdb ?? 0])) as $imdbId)
            <li class="meta__letterboxd">
                <a class="meta-id-tag"
                    ...
                    title="Letterboxd" target="_blank">
        ...
        <section class="meta__chip-container">
            ...
            @foreach ($meta?->credits?->where('occupation_id', '=', App\Enums\Occupation::ACTOR->value)?->sortBy('order') ?? [] as $credit)
                <article class="meta-chip-wrapper">
                    <a href="{{ route('mediahub.persons.show', ['id' => $credit->person->id, 'occupationId' => $credit->occupation_id]) }}"
                        class="meta-chip">
                        ...
                        @else
                            <i
                                class="{{ config('other.font-awesome') }} fa-user meta-chip__icon"></i>
                        @endif
                ...
                <article class="meta__collection">
                    <details>
                        <summary class="meta-chip">
                            <i
                                class="{{ config('other.font-awesome') }} fa-rectangle-history meta-chip__icon"></i>
                        ...
                        <div class="meta-chip__list">
                            <article class="meta__collection-item">
                                <a class="meta-chip meta-chip--value-only"
                            ...
                            @foreach ($meta?->collections?->first()->movies as $movie)
                                <article class="meta__collection-item">
                                    <a class="meta-chip meta-chip--value-only"
                                        ...
                                        @else
                                            <i
                                                class="{{ config('other.font-awesome') }} fa-film meta-chip__icon"></i>
                                        @endif
                                        @if ($movie->is($meta))
                                            <h3 class="meta-chip__value">
                                                <strong>
