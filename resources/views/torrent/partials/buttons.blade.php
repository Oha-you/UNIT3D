<menu class="torrent__buttons form__group--short-horizontal">
    ...
    @if ($fileExists && $torrent->status === \App\Enums\ModerationStatus::APPROVED)
        @if (
            ...
            <li class="form__group form__group--short-horizontal">
                <form action="{{ route('freeleech_token', ['id' => $torrent->id]) }}"
                    method="POST" style="display: contents" x-data="freeleechTokenConfirmation">
                    @csrf
            ...
            <dialog class="dialog dialog--auto-width" x-bind="dialogElement">
                ...
                <div class="dialog__form" x-bind="dialogForm">
                    <div class="bbcode-rendered" style="text-align: left">
                        <pre class="torrent__nfo-pre"><code class="torrent__nfo">{{ iconv('cp437', 'utf8', $torrent->nfo) }}</code></pre>
                    </div>
                </div>
            </dialog>
        </li>
            ...
            <dialog class="dialog" x-bind="dialogElement">
                ...
                <form class="dialog__form" method="POST"
                    ...
                    <div class="form__group">
                        <input id="bon" class="form__text" list="torrent_quick_tips"
                            name="bon" placeholder=" " type="text" pattern="[0-9]*"
                            inputmode="numeric" />
    ...
    <li x-data="dialog" class="form__group form__group--short-horizontal">
        <button class="form__button form__button--outlined form__button--centered"
            title="{{ __('common.edit') }}" x-bind="showDialog">
                    ...
                    @if ($torrent->folder !== null)
                        <span
                            ...
                            <i class="{{ config('other.font-awesome') }} fa-folder"
                                style="grid-area: icon; padding-right: 4px"></i>
                            {{-- format-ignore-start --}}<span
                                style="padding-right: 4px; word-break: break-all">{{ $torrent->folder }}</span>{{-- format-ignore-end --}}
    ...
    @if ($torrent->status === \App\Enums\ModerationStatus::APPROVED && $user->playlists->count() > 0)
        <li x-data="dialog" class="form__group form__group--short-horizontal">
            ...
            <dialog class="dialog" x-bind="dialogElement">
                ...
                <form class="dialog__form" method="POST"
                    action="{{ route('playlist_torrents.store') }}" x-bind="dialogForm">
                    @csrf
                    <input type="hidden" name="torrent_id" value="{{ $torrent->id }}" />
                    <p class="form__group">
    ...
    @if (
        ...
        <li class="form__group form__group--short-horizontal">
            <form action="{{ route('reseed', ['id' => $torrent->id]) }}" method="POST"
    ...
    @if ($user->group->is_modo)
            ...
            <li x-data="dialog" class="form__group form__group--short-horizontal">
                ...
                <dialog class="dialog" x-bind="dialogElement">
                    ...
                    <form class="dialog__form" method="POST"
                        ...
                        <input type="hidden" name="torrent_id" value="{{ $torrent->id }}" />
                        <p class="form__group">
                            <textarea id="reason" class="form__textarea" name="reason" required></textarea>
                            <label for="trump_reason" class="form__label form__label--floating"
                                for="reason">
                                {{ __('common.reason') }}
                            </label>
                        </p>
        ...
        @else
            <li class="form__group form__group--short-horizontal">
                <form action="{{ route('torrent.trump.destroy', ['torrent' => $torrent]) }}"
