<li class="form__group form__group--short-horizontal" x-data="dialog">
    ...
    <dialog class="dialog" x-bind="dialogElement">
        ...
        <form class="dialog__form" method="POST"
            ...
            x-bind="dialogForm">
            @csrf
            <input id="type" type="hidden" name="request_id"
                value="{{ $torrentRequest->id }}" />
            <p class="form__group">
                <input id="seedbonus" class="form__text" inputmode="numeric" name="seedbonus"
                    pattern="[0-9]*?[1-9][0-9]{2,}" placeholder=" " type="text" />
            ...
            <p class="form__group">
                <button class="form__button form__button--filled"
                    @if ($user->seedbonus < 100) disabled
                        title="{{ __('request.dont-have-bps') }}" @endif>
                    {{ __('request.vote') }}
                </button>
