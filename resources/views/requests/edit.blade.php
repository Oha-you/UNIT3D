            ...
            <script nonce="{{ HDVinnie\SecureHeaders\SecureHeaders::nonce('script') }}">
                document.addEventListener('alpine:init', () => {
                    Alpine.data('requestEdit', () => ({
                        ...
                        tmdb_movie_exists: {{
                    Js::from(
                        old('movie_exists_on_tmdb', $torrentRequest->tmdb_movie_id) !== null,
                    )
                }},
                        tmdb_tv_exists: {{
                    Js::from(
                        old('tv_exists_on_tmdb', $torrentRequest->tmdb_tv_id) !== null,
                    )
                }},
                        imdb_title_exists: {{
                    Js::from(
                        old('title_exists_on_imdb', $torrentRequest->imdb) !== null,
                    )
                }},
                        tvdb_tv_exists: {{
                    Js::from(
                        old('tv_exists_on_tvdb', $torrentRequest->tvdb) !== null,
                    )
                }},
                        mal_anime_exists: {{
                    Js::from(
                        old('anime_exists_on_mal', $torrentRequest->mal) !== null,
                    )
                }},
                        igdb_game_exists: {{
                    Js::from(
                        old('game_exists_on_igdb', $torrentRequest->igdb) !== null,
                    )
                }},
                    }));
                });
