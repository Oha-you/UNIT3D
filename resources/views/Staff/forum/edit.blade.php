            ...
            <form class="form" method="POST"
                ...
                <p class="form__group">
                    <input id="position" class="form__text" inputmode="numeric" name="forum[position]"
                        pattern="[0-9]*" placeholder=" " type="text" value="{{ $forum->position }}"
                        required />
    ...
    {{ $forum->description }}</textarea>
                ...
                <p class="form__group">
                    <select id="forum_category_id" name="forum[forum_category_id]" class="form__select">
                        @foreach ($categories as $category)
                            <option value="{{ $category->id }}" @selected($category->id === $forum->category->id)>
                                {{ $category->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <p class="form__group">
                    <select id="default_topic_state_filter" name="forum[default_topic_state_filter]"
                        ...
                        <option value="open" @selected($forum->default_topic_state_filter === 'open')>
                            {{ __('forum.open') }}
                        </option>
                        <option value="close" @selected($forum->default_topic_state_filter === 'close')>
                            {{ __('forum.closed') }}
                        </option>
                    </select>
                                ...
                                @foreach ($groups as $group)
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            <input type="checkbox"
                                                name="permissions[{{ $loop->index }}][read_topic]"
                                                value="1" @checked($forum->permissions->where('group_id', '=', $group->id)->first()?->read_topic) />
                                        </td>
                                        <td>
                                            <input type="hidden"
                                            ...
                                            <input type="checkbox"
                                                name="permissions[{{ $loop->index }}][start_topic]"
                                                value="1" @checked($forum->permissions->where('group_id', '=', $group->id)->first()?->start_topic) />
                                        </td>
                                        <td>
                                            <input type="hidden"
                                            ...
                                            <input type="checkbox"
                                                name="permissions[{{ $loop->index }}][reply_topic]"
                                                value="1" @checked($forum->permissions->where('group_id', '=', $group->id)->first()?->reply_topic) />
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
