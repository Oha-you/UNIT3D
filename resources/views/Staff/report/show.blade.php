...
@section('main')
    ...
    <section class="panelV2">
        ...
        {{-- format-ignore-start --}}<div class="panel__body" style="white-space: pre-wrap">
            {{ $report->message }}</div>{{-- format-ignore-end --}}
    </section>
    @if (count($urls) > 0)
        <section class="panelV2">
    ...
    @if ($report->solved_by !== null)
        <section class="panelV2">
            ...
            {{-- format-ignore-start --}}<div class="panel__body" style="white-space: pre-wrap">
                {{ $report->verdict }}</div>{{-- format-ignore-end --}}
        </section>
    @else
        <section class="panelV2">
    ...
    <section class="panelV2">
        ...
        <div class="panel__body">
            <form class="form form--horizontal"
                ...
                @csrf
                <p class="form__group">
                    <select id="assigned_to" name="assigned_to" class="form__select"
                        x-on:change="$root.submit()">
                        <option hidden disabled selected value=""></option>
                        @foreach ($staff as $staffUser)
                            <option value="{{ $staffUser->id }}" @selected($staffUser->id === $report->assigned_to)>
                                {{ $staffUser->username }}
                            </option>
                        @endforeach
                    </select>
            ...
            @if ($report->assigned_to !== null)
                <form action="{{ route('staff.reports.assignee.destroy', ['report' => $report]) }}"
                    method="POST">
                    @csrf
                    @method('DELETE')
        ...
        <div class="panel__body">
            @if ($report->snoozed_until === null)
                <form class="form"
                    ...
                    method="POST" x-data x-on:change="$root.submit()">
                    @csrf
                    <p class="form__group">
                        <input id="snoozed_days" class="form__text" name="snoozed_days" placeholder=" "
                    ...
                    <div class="form__group--short-horizontal">
                        <p class="form__group form__group--short-horizontal">
                            <button name="snoozed_until" value="{{ now()->addDays(1) }}"
                                class="form__button form__button--outlined form__button--centered">
                                1 day
                            </button>
                        </p>
                        <p class="form__group form__group--short-horizontal">
                            <button name="snoozed_until" value="{{ now()->addDays(3) }}"
                                class="form__button form__button--outlined form__button--centered">
                                3 days
                            </button>
                        </p>
                        <p class="form__group form__group--short-horizontal">
                            <button name="snoozed_until" value="{{ now()->addDays(7) }}"
                                class="form__button form__button--outlined form__button--centered">
                                1 week
                            </button>
                        </p>
                        <p class="form__group form__group--short-horizontal">
                            <button name="snoozed_until" value="{{ now()->addDays(14) }}"
                                class="form__button form__button--outlined form__button--centered">
                                2 weeks
                            </button>
                        </p>
                        <p class="form__group form__group--short-horizontal">
                            <button name="snoozed_until" value="{{ now()->addDays(28) }}"
                                class="form__button form__button--outlined form__button--centered">
                                4 weeks
                            </button>
                        </p>
                        <p class="form__group form__group--short-horizontal">
                            <button name="snoozed_until" value="{{ now()->addDays(56) }}"
                                class="form__button form__button--outlined form__button--centered">
                                8 weeks
                            </button>
                        </p>
