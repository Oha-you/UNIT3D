...
<li class="nav-tab-menu">
    <a class="{{ Route::is('users.show', 'users.edit') ? 'nav-tab--active__link' : 'nav-tab__link' }}"
        href="{{ route('users.show', ['user' => $user]) }}">
        {{ __('user.profile') }}
    </a>
    <ul class="nav-tab-menu__items">
        <li class="{{ Route::is('users.show') ? 'nav-tab--active' : 'nav-tavV2' }}">
            <a class="{{ Route::is('users.show') ? 'nav-tab--active__link' : 'nav-tab__link' }}"
                href="{{ route('users.show', ['user' => $user]) }}">
                {{ __('user.profile') }}
            </a>
        </li>
        @if ($isProfileOwner)
            <li class="{{ Route::is('users.edit') ? 'nav-tab--active' : 'nav-tavV2' }}">
                <a class="{{ Route::is('users.edit') ? 'nav-tab--active__link' : 'nav-tab__link' }}"
        ...
        @else
            <li class="nav-tabV2">
                @if ($user->followers()->where('users.id', '=', auth()->id())->exists())
                    <form action="{{ route('users.followers.destroy', ['user' => $user]) }}"
...
@if ($isProfileOwner || $isModo)
    <li class="nav-tab-menu">
        <a class="{{ Route::is('users.general_settings.edit', 'users.email.edit', 'users.password.edit', 'users.passkeys.index', 'users.rsskeys.index', 'users.apikeys.index', 'users.two_factor_auth.edit', 'users.privacy_settings.edit', 'users.notification_settings.edit') ? 'nav-tab--active__link' : 'nav-tab__link' }}"
        ...
        <ul class="nav-tab-menu__items">
            @if ($isProfileOwner)
                <li
                    ...
                    <a class="{{ Route::is('users.general_settings.edit') ? 'nav-tab--active__link' : 'nav-tab__link' }}"
                        href="{{ route('users.general_settings.edit', ['user' => $user]) }}">
                        {{ __('user.general') }}
                    </a>
                </li>
            ...
            <li
                ...
                <a class="{{ Route::is('users.two_factor_auth.edit') ? 'nav-tab--active__link' : 'nav-tab__link' }}"
            ...
            @if ($isProfileOwner)
                <li
                    ...
                    <a class="{{ Route::is('users.privacy_settings.edit') ? 'nav-tab--active__link' : 'nav-tab__link' }}"
                ...
                <li
                    ...
                    <a class="{{ Route::is('users.notification_settings.edit') ? 'nav-tab--active__link' : 'nav-tab__link' }}"
...
@if ($isProfileOwner || $isModo)
    <li class="nav-tab-menu">
        @if ($isProfileOwner || $isModo)
            <a class="{{ Route::is('users.history.index', 'users.torrents.index', 'users.peers.index', 'users.resurrections.index') ? 'nav-tab--active__link' : 'nav-tab__link' }}"
        ...
        <ul class="nav-tab-menu__items">
            @if ($isProfileOwner || $isModo)
                <li
                    ...
                    <a class="{{ Route::is('users.history.index') ? 'nav-tab--active__link' : 'nav-tab__link' }}"
                        href="{{ route('users.history.index', ['user' => $user]) }}">
                        {{ __('torrent.history') }}
                    </a>
                </li>
                ...
                <li class="{{ Route::is('users.peers.index') ? 'nav-tab--active' : 'nav-tavV2' }}">
                    <a class="{{ Route::is('users.peers.index') ? 'nav-tab--active__link' : 'nav-tab__link' }}"
                ...
                <li
                    ...
                    <a class="{{ Route::is('users.resurrections.index') ? 'nav-tab--active__link' : 'nav-tab__link' }}"
            ...
            @if ($isProfileOwner || $isModo)
                ...
                <li
                    ...
                    <a class="{{ Route::is('users.unregistered_info_hashes.index') ? 'nav-tab--active__link' : 'nav-tab__link' }}"
            ...
            @if ($isProfileOwner)
                @if (!config('announce.external_tracker.is_enabled'))
                    <form action="{{ route('users.peers.mass_destroy', ['user' => $user]) }}"
                        method="POST" style="display: contents">
                        @csrf()
                        @method('DELETE')
                        <button class="nav-tab__link" type="submit">
                        ...
                        <form class="dialog__form"
                            ...
                            <fieldset class="form__fieldset">
                                ...
                                <div class="form__group">
                                    <input class="form__radio" type="radio" id="history"
                                        name="type" value="false" checked />
                                    <label for="history" class="form__label">All history</label>
                                </div>
                                <div class="form__group">
                                    <input class="form__radio" type="radio" id="peer"
    ...
    <ul class="nav-tab-menu__items">
        @if (auth()->user()->isAllowed($user, 'achievement', 'show_achievement'))
            <li
                ...
                <a class="{{ Route::is('users.achievements.index') ? 'nav-tab--active__link' : 'nav-tab__link' }}"
        ...
        @if (auth()->user()->isAllowed($user, 'forum', 'show_topic'))
            <li class="{{ Route::is('users.topics.index') ? 'nav-tab--active' : 'nav-tavV2' }}">
                <a class="{{ Route::is('users.topics.index') ? 'nav-tab--active__link' : 'nav-tab__link' }}"
        ...
        @if (auth()->user()->isAllowed($user, 'forum', 'show_post'))
            <li class="{{ Route::is('users.posts.index') ? 'nav-tab--active' : 'nav-tavV2' }}">
                <a class="{{ Route::is('users.posts.index') ? 'nav-tab--active__link' : 'nav-tab__link' }}"
        ...
        @if (auth()->user()->isAllowed($user, 'follower', 'show_follower'))
            <li
                ...
                <a class="{{ Route::is('users.followers.index') ? 'nav-tab--active__link' : 'nav-tab__link' }}"
        ...
        @if (auth()->user()->group->is_modo || auth()->id() === $user->id)
            <li
                ...
                <a class="{{ Route::is('users.following.index') ? 'nav-tab--active__link' : 'nav-tab__link' }}"
...
@if ($isProfileOwner || $isModo)
    <li class="nav-tab-menu">
        <a class="{{ Route::is('users.earnings.index', 'users.transactions.create', 'users.gifts.index', 'users.gifts.create', 'users.post_tips.index', 'users.torrent_tips.index') ? 'nav-tab--active__link' : 'nav-tab__link' }}"
            href="{{ route('users.earnings.index', ['user' => $user]) }}">
            {{ __('bon.bonus') }} {{ __('bon.points') }}
        </a>
        <ul class="nav-tab-menu__items">
            <li
                class="{{ Route::is('users.earnings.index') ? 'nav-tab--active' : 'nav-tavV2' }}">
                <a class="{{ Route::is('users.earnings.index') ? 'nav-tab--active__link' : 'nav-tab__link' }}"
                    href="{{ route('users.earnings.index', ['user' => $user]) }}">
                    {{ __('bon.earnings') }}
                </a>
            </li>
            @if ($isProfileOwner)
                <li
                    ...
                    <a class="{{ Route::is('users.transactions.create') ? 'nav-tab--active__link' : 'nav-tab__link' }}"
            ...
            <li class="{{ Route::is('users.gifts.index') ? 'nav-tab--active' : 'nav-tavV2' }}">
                <a class="{{ Route::is('users.gifts.index') ? 'nav-tab--active__link' : 'nav-tab__link' }}"
                    href="{{ route('users.gifts.index', ['user' => $user]) }}">
                    {{ __('bon.gifts') }}
                </a>
            </li>
            <li
                class="{{ Route::is('users.post_tips.index') ? 'nav-tab--active' : 'nav-tavV2' }}">
                <a class="{{ Route::is('users.post_tips.index') ? 'nav-tab--active__link' : 'nav-tab__link' }}"
            ...
            <li
                ...
                <a class="{{ Route::is('users.torrent_tips.index') ? 'nav-tab--active__link' : 'nav-tab__link' }}"
        ...
        <ul class="nav-tab-menu__items">
            @if ($isProfileOwner || $isModo)
                <li
                    ...
                    <a class="{{ Route::is('users.wishes.index') ? 'nav-tab--active__link' : 'nav-tab__link' }}"
                ...
                <li
                    ...
                    <a class="{{ Route::is('users.seedboxes.index') ? 'nav-tab--active__link' : 'nav-tab__link' }}"
                ...
                <li
                    ...
                    <a class="{{ Route::is('users.invites.index') ? 'nav-tab--active__link' : 'nav-tab__link' }}"
                ...
                <li
                    ...
                    <a class="{{ Route::is('users.invite_tree.index') ? 'nav-tab--active__link' : 'nav-tab__link' }}"
